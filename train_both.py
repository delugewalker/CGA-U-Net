# python3 -m torch.distributed.launch --nproc_per_node=4 --master_port 20004 train_both.py
...
from utils.loss import DiceLoss, AffinityLoss, AffinityLoss_small, softmax_dice
from utils.Construct_affinity import one_hot, reconstruct_one_hot
...
parser.add_argument('--experiment', default='Unet_Both_', type=str)

parser.add_argument('--date', default=local_time.split(' ')[0], type=str)

parser.add_argument('--description',
                    default='Unet with supervised attention and interclass distance, using new update method'
...
parser.add_argument('--model_name', default='Unet_Both', type=str)
...
parser.add_argument('--gpu', default='3,4,5,6', type=str)
...
parser.add_argument('--end_epoch', default=550, type=int)

parser.add_argument('--save_freq', default=100, type=int)

parser.add_argument('--resume', default='model_epoch_199.pth', type=str)
...
def main_worker():
    ...
    for epoch in range(args.start_epoch, args.end_epoch):
        ...
        for i, data in enumerate(train_loader):
            ...
            aff_label = one_hot(target, classes=4)  # (B, Cl, H, W, D)

            output, attention, Euclidean_dis, weight = model(x)
            loss, loss_main, loss_att, loss1, loss2, loss3 = criterion(output, attention, target, aff_label)
            Euclidean_loss = 10 * torch.log((1e+2 / Euclidean_dis)+1)

            reduce_weight = all_reduce_tensor(weight, world_size=num_gpu).data.cpu().numpy()
            # print('weight:', weight, 'reduce_weight:', reduce_weight)
            reduce_loss = all_reduce_tensor(loss, world_size=num_gpu).data.cpu().numpy()
            reduce_loss_main = all_reduce_tensor(loss_main, world_size=num_gpu).data.cpu().numpy()
            reduce_loss_att = all_reduce_tensor(loss_att, world_size=num_gpu).data.cpu().numpy()
            ...
            if epoch < 20:
                ...
                loss_all = loss + Euclidean_loss

            if args.local_rank == 0:
                logging.info('Epoch:{}_Iter:{}  loss:{:.5f} || main:{:.5f} || att:{:.5f} || loss1:{:.4f} | loss2:{:.4f} | loss3:{:.4f} || weight:{:.4f}'
                             .format(epoch, i, reduce_loss, reduce_loss_main, reduce_loss_att, reduce_loss1, reduce_loss2, reduce_loss3, reduce_weight[0]))
        ...
        if args.local_rank == 0:
            ...
            writer.add_scalar('Euclidean_dis:', Euclidean_dis, epoch)
