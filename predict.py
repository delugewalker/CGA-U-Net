...
def validate_softmax(
    ...
    for i, data in enumerate(valid_loader):
        ...
        if multimodel:
            ...
            load_file1 = load_file.replace('last', '548')
            if os.path.isfile(load_file1):
                ...
                logit = F.softmax(logit, dim=1)
                output += logit / 4.0
            load_file1 = load_file.replace('last', '547')
            if os.path.isfile(load_file1):
                checkpoint = torch.load(load_file1)
                model.load_state_dict(checkpoint['state_dict'])
                print('Successfully load checkpoint {}'.format(load_file1))
                ...
                logit = F.softmax(logit, dim=1)
                output += logit / 4.0
            load_file1 = load_file.replace('last', '546')
            if os.path.isfile(load_file1):
                checkpoint = torch.load(load_file1)
                model.load_state_dict(checkpoint['state_dict'])
                print('Successfully load checkpoint {}'.format(load_file1))
        ...
        if heatmap_use:
            ...
            for slice in range(0, heatmap.shape[-1]):
                ...
                # heatmap_slice = cv2.resize(heatmap_slice, (240, 240), interpolation=cv2.INTER_LINEAR)
